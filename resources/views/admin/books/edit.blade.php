                    ...
                    <form action="{{ route('books.update', $book->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" 
                                    class="form-control @error('phone_author') is-invalid @enderror" 
                                    name="phone_author" value="{{ old('phone_author', $book->phone_author) }}">
                                @error('phone_author')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select class="form-control select2 @error('categoryB_id') is-invalid @enderror" name="categoryB_id">
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" 
                                            {{ old('categoryB_id', $book->categoryB_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                @if ($book->tumbnail)
                                    ...
                                    <img src="{{ asset('storage/' . $book->tumbnail) }}" alt="Thumbnail" width="100">
                                @endif
                                @error('tumbnail')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
